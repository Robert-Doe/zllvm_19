...
static bool checkArgCount(Sema &S, CallExpr *call, unsigned desiredArgCountMin,
                          unsigned desiredArgCountMax) {
  unsigned argCount = call->getNumArgs();
  if (argCount >= desiredArgCountMin && argCount <= desiredArgCountMax)
    return false;
...
// from zeno: functions to pay attention to regarding handling of
// PseudoInstructions:
